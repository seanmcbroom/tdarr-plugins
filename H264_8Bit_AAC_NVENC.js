...
const details = () => ({
  ...
  const plugin = (file, librarySettings, inputs, otherArguments) => {
    ...
    // If audio is aac, we don't need to do anything
    if (!isAAC) {
      response.infoLog += '☒File has no aac track \n';
      response.preset = ', -map 0:v -sn -map 0:s? -map 0:a -c:v copy -c:a aac -c:s copy';
    ...
    + `-map 0:a -c:v ${videoCodec} -crf 13 -vf format=yuv420p `
    + `-c:a aac -c:s copy ${extraArguments}`;
    response.processFile = true;
