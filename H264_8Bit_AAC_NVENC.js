...
const details = () => ({
  ...
  Version: '1.00',
  Tags: 'pre-processing,ffmpeg,nvenc h264,configurable',
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  let videoCodecEncoder = "libx264"

  if (otherArguments['nodeHardwareType'] == "nvenc") {
    videoCodecEncoder = "h264_nvenc"
  }

  // Set up required variables.
  let videoIdx = 0;
  let extraArguments = '';
  let audioCodecEncoder = 'copy';
  let isAAC = false;
  let isH264 = false;
  ...
  if (isH264 && isAAC && !isCorrectContainer) {
  ...
  response.preset += `,-map 0:v -map 0:s? -sn `
  + `-map 0:a -c:v ${videoCodecEncoder} -crf 13 -vf format=yuv420p `
  + `-c:a ${audioCodecEncoder} -c:s copy ${extraArguments}`;
  response.processFile = true;
